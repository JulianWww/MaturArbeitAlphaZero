...
\newcommand{\imgRef}[1]{(\imgRefInline{#1})}
\newcommand{\imgRefInline}[1]{fig \ref{#1} on page \pageref{#1}}
...
In this example player \(X\) is allowed to place a stone in anny empy field \(\mathbb A(s) = \left\{ 2, 3, 4, 5, 6, 7, 8 \right\}\). Therefor \(\mathcal A(s, a)\) is valid if \(a \in \mathbb A(s)\) and otherwise invalid. Therefor \(\mathcal A(s, 0)\) and \(\mathcal A(s, 1)\) are invalid while  \(\mathcal A(s, 2),\,\dots,\,\mathcal A(s, 8)\) are valid.
...
Q(e) = \frac{W(e)}{N(e)}
\end{equation}
The fourth and last of these functions is \(P~:~\mathbb{E}\to\mathbb{R}\). \(P\) is the policy function, it's the neural network's preliminary estimation of: 
\begin{equation}\label{eq:MCTS:P}
	P(e) \approx \frac{N(e)}{\sum_{i\in\mathbb{E}(\mathcal{N}_{from}(e))}N(i)}
\end{equation}
...
\]
where \(a\) is the edges action \(a \in \mathbb{A}(n_L)\). \(\mathbb M\) and \(\mathbb E(n_L\) are then redefined as \(\mathbb M = \mathbb M \cup \mathbb M_{new}\) and \(\mathbb E(n_L) = \{\mathcal E(n_L, a)\;|\;a\in\mathbb A(n_L)\}\). \(M_{expanded}\) and \(M_{leaf}\) also change according to there definition.
...
\end{align*}
See \imgRefInline{MCTS-simulation} for example Simulation.
